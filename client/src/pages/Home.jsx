...
export default function Home() {
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        const res = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/course/getcourses?limit=4`);
        const data = await res.json();
        setCourses(data.courses);
      } catch (err) {
        setErrorCourses("Failed to fetch courses");
  ...
  useEffect(() => {
    const fetchYoutubeVideos = async () => {
      try {
        const res = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/youtube-videos/getvideos?limit=4`);
        const data = await res.json();
